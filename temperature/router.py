...
async def get_temperatures(params_db: CommonDB, params_limit: CommonLimitation):
    return await crud.get_temperature(db=params_db, **params_limit)


@router.get("/temperatures/", response_model=schemas.Temperature)
async def get_temperature_by_city_id(
    params_db: CommonDB, params_id: CommonParametersWithId
):
    temperature = await crud.get_temperature_by_city(db=params_db, city_id=params_id)
...
async def update_temperature(db: CommonDB):
    invalid_cities, valid_cities = await crud.updates_temperature(db=db)
    message = generate_main_message(invalid_cities=invalid_cities, valid_cities=valid_cities)
