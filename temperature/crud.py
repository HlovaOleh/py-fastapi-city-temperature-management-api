...
from temperature.utils import generate_main_temperature_data


async def updates_temperature(db: AsyncSession):
    invalid_cities = []
    valid_cities = []
    ...
    async with httpx.AsyncClient as client:
        ...
        for city in cities:
            temperature = await generate_main_temperature_data(city=city, client=client)
...
async def get_temperature(db: AsyncSession, skip: int = 0, limit: int = 0) -> List[schemas.Temperature]:
...
async def get_temperature_by_city(db: AsyncSession, city_id: int) -> schemas.Temperature:
