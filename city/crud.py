...
async def get_cities(
    db: AsyncSession, skip: int = 0, limit: int = 0
) -> List[schemas.City]:
...
async def get_city_by_id(db: AsyncSession, city_id: int) -> schemas.CityDetail:
    query = (
        select(models.City)
...
async def post_city(db: AsyncSession, city: schemas.CityCreate) -> schemas.City:
    query = select(models.City).where(models.City.name == city.name)
...
async def delete_city(db: AsyncSession, city_id: int) -> schemas.CityDetail:
    city = await get_city_by_id(db, city_id)
